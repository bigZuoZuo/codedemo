...
import {
  ...
  getCountryAndTownDatasByTenant
...
class DivisionSelect extends Taro.Component {
  ...
  async setLocationAddress() {
    ...
    if (openDivision == null) {
      ...
      const countryResult = await getChildren(currentDivisionCode.substr(0, 4) + '00000000', false)
      this.setState({
        countries: get(countryResult, 'data', []).filter(item => item.status === 'ACTIVE'),
      })
  ...
  async selectCity(division: Division) {
    let countryAndTowns = await getCountryAndTownDatasByTenant(division.code);
    const countryResult = await getChildren(division.code.substr(0, 4) + '00000000', false)
    this.setState({
      selectCity: division,
      countries: get(countryResult, 'data', []).filter(item => item.status === 'ACTIVE'),
      selectCountry: {
    ...
    this.setState({
      selectDivisionList: get(countryAndTowns, 'data', [])
    });
  }
  ...
  async selectCountry(selectCountry: Division) {
    ...
    let towns = await getCountryAndTownDatasByTenant(selectCountry.code);
    this.setState({
      selectDivisionList: get(towns, 'data', [])
    });
  }
