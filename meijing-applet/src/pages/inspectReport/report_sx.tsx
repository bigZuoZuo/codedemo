...
export default class InspectReply extends Component<InspectReplyProps, InspectReplyState> {
    ...
    report = async () => {
        ...
        if (photos && photos.length > 0) {
            ...
            const errorUploadResults:UploadResult[] = imageUploadResults.filter(re=>!re.success);

            if(errorUploadResults.length > 0) {
                this.showErrorMsg(errorUploadResults.map(item=> item.message));
                    ...
                    <View className='replyinfoView'>
                        ...
                        {type === InspectInfoType.INCIDENT &&
                            <Block>
                                ...
                                <View className='info_item'>
                                    <Text className='item_left required'>事件类型</Text>
                                    <View className='item_right'>
                                        ...
                                        <FpiPopupChoose range={pollutionTypeList} rangeKey='name'typeName='请选择事件类型' value={get(pollutionType, 'id', '')} onChange={this.onPollutionTypeChange.bind(this)}>
