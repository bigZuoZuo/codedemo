...
interface Props {
  ...
  onSiteTypeChange: (selectSiteTypeIds: string[]) => void;
...
export default class DispatchDrawer extends Taro.Component<Props> {
  ...
  onSiteTypeClick = (siteType: SiteType) => {
    ...
    if (status.selectSiteTypes) {
      if (status.selectSiteTypes.includes(siteType.code)) {
        selectTypes = status.selectSiteTypes.filter((typeId) => typeId != siteType.code)
      } else {
        selectTypes = [...status.selectSiteTypes, siteType.code]
      }
    } else {
      selectTypes = [siteType.code]
    }
            ...
            <View className="factor_group">
              {
                stationTypes.map((siteType) => {
                  return (
                    <View key={siteType.id} className={`content ${status.selectSiteTypes.includes(siteType.code) ? 'active' : ''}`}>
