...
import { View, Map, Text, Image, Button, CoverView, CoverImage } from '@tarojs/components'
import './index.scss'
import { DivisionMonitorData , Location, Inspect, Staff } from '../../model'
...
//研判工具左图
const yanpanLeft = rootSourceBaseUrl + "/assets/task_dispatch/yan-pan-tool.png";
//研判工具右图
const yanpanRight = rootSourceBaseUrl + "/assets/task_dispatch/yan-pan-right.png";
//研判工具左图
const yanpanMenuLeft = rootSourceBaseUrl + "/assets/task_dispatch/yan-pan-left.png";
//站点溯源
...
class TaskDispatch extends Taro.Component {
    ...
    onBindMarker(res) {
        ...
        const { dispatchStore } = this.props
        const markerId =  res.target.markerId || res.markerId
        dispatchStore.selectMarker(markerId)
        this.setState({
            isShowAiMsg: false,
            factorListHeight: 0
        })
    ...
    componentWillMount() {
        try {
            ...
            getStationType().then((response) => {
                response.data.map((siteType: SiteType) => {
    ...
    componentDidMount() {
        const { userStore: { userDetails }, dispatchStore } = this.props
        dispatchStore.loadLatestMonitorDatas(userDetails.divisionCode,false)
    }
    ...
    render() {
        const {
            userStore: {
                userDetails: { divisionName, divisionFree, hasOpenCar }
            },
            dispatchStore: {
                remind, controlAreas, drawerStatus,
                siteMarkers, inspectMarkers, pollutionSourceMarkers, staffMarkers, isInspectSelected, isPollutionSourceSelected, isSiteSelected, isStaffSelected,
                selectedSite, selectedSiteMonitorData, selectedInspect, selectedPollutionSource, selectedStaff, currentStaffAddress,
                currentStaffDistance, positionCurrentLocation, positionCurrentLocationMarker
            }
        } = this.props;
        ...
        const { isRespondDispatchShow, mariginLeft, isMapShadeShow, stationTypes,
            isAtDrawerShow, pollutantTypes, pollutantSourceTypes, isTurnDown, centerLocaltion, factorListHeight ,isShowMenu} = this.state;
            ...
            <View className='root_view'>
                <View className='map_group' style={{ marginLeft: `${mariginLeft}rpx`, transition: `all 0.4s` }}>
                    <View className='map_content'>
                        ...
                        <Map style={{ height: `calc(100%)`, zIndex: 1, width: `100%` }}
                          ...
                          latitude={centerLocaltion.latitude}
                        />
                        <View style='display:none;' onClick={this.onRegionchange.bind(this)}></View>
                    ...
                    <View className="dispatch_item">
                        ...
                        {!isAtDrawerShow && !divisionFree &&
                            <CoverView className='left_control_group' onClick={this.switchItem}>
                                <CoverView className='left_image'>
                                    <CoverImage className='image' src={yanpanLeft}></CoverImage>
                                </CoverView>
                                <CoverView className='content_detail'>研判分析工具</CoverView>
                                <CoverView className='right_image'>
                                    <CoverImage className='image' src={yanpanRight}></CoverImage>
                                </CoverView>
                            </CoverView>}
                        {!isAtDrawerShow && isShowMenu &&
                            <CoverView className='left_menu'>
                                <CoverView className='menu'>
                                {
                                ANALYSIS_MENUS.map((menu, index) => (
                                    <CoverView key={menu.code + index} className='item' onClick={() => { this.jumpToOther(menu) }}>
                                        <CoverImage className='image' src={menu.image}></CoverImage>
                                        <CoverView className='title'>{menu.title}</CoverView>
                                    </CoverView>
                                ))
                                }
                                </CoverView>
                                <CoverView className='right'>
                                    <CoverImage className='image' src={yanpanMenuLeft} onClick={this.switchItem}></CoverImage>
                                </CoverView>
                            </CoverView>}
