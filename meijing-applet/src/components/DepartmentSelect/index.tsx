...
import { hierarchyList } from '../../service/department'
...
class Index extends Component<DepartmentProps, DepartmentState> {
    ...
    getInitData = async () => {
        ...
        const {type,divisionCode}  = this.props;
        let departmentData = {}
        if(type!==''){
           departmentData = await hierarchyList(divisionCode,false,)
        }else{
           departmentData = await hierarchyList(divisionCode)
        }
