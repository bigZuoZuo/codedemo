...
export default class TodayItem extends Component<TodayItemProps, TodayItemState> {
    ...
    onDetail = () => {
        const { data, dataType = 0, appType = 'construction' } = this.props
        let url: any = null
        if (dataType) {
            url = `/pages/works/detail?inspectId=${data.id}&from=examine`
        }
        else {
            url = `/pages/${appType === 'construction' ? 'work_options' : 'works'}/edit_event?inspectId=${data.id}`
        }
        Taro.navigateTo({ url })
    }
        ...
        return (
            <View className='list-item' onClick={this.onDetail}>
                ...
                <View className='right'>
                    <Image className='img' src={get(data, 'pictureLinks[0]', IconPlaceHolder)} />
