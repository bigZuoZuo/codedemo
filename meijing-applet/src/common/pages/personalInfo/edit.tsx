...
import {User,updateCurrentUser} from '../../service/user'
...
import { UserDetails, UserStore } from "../../store/user"
...
export default class Index extends Component<MyProps, MyState> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      name: userDetails.tenantUser.userName,
      nickname: userDetails.simpleUserInfo.nickName,
      phone: userDetails.simpleUserInfo.phone,
      avatar: userDetails.avatar,
      divisionCode: userDetails.pollutionSourceInfo.divisionCode,
      department: userDetails.departmentNode,
      divisionName: userDetails.pollutionSourceInfo.divisionName || '',
  ...
  async componentDidMount () { 
    ...
    try {
      const resDepartment = await treeByTenantCode(userDetails.tenant.code);
  ...
  render () {
    const {nickname, name, divisionName, department,  roles} = this.state;
    const { userStore : { userDetails}} = this.props
    const { simpleUserInfo } = userDetails;
          ...
          <View className='userinfo_panel'>
            <View className='info_item'>
                ...
                <View className='item_right' onClick={this.handleAvatarChange.bind(this)}>
                    <AtAvatar className='avatar' circle image={`${getUserAvatarUrl(simpleUserInfo && simpleUserInfo.userId)}`} />
                </View>
            </View>
