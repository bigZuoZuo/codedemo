...
import { Card, Col, Row, message, Button, Form, Select, Popconfirm } from 'antd';
...
import { DataSourceType, NameCode, FormDataTy } from './data';
...
const AdvancedForm: FC<Record<string, any>> = (props) => {
  ...
  const [dataList, setDataList] = useState<FormDataTy>({});
  ...
  const [columns, setColumns] = useState<ProColumns<DataSourceType>[]>([
    ...
    },
    {
      ...
      renderFormItem: (text, record) => {
        ...
        let number = totalNumber(record.record);
        return number;
      },
    },
    {
  ...
  const onFinish = async (values: Record<string, any>) => {
    ...
    const res = await request<API.GeneralInterface>('/web/schoolDetails', {
      ...
      data: {
        ...
        version: dataList.version || undefined,
  ...
  /** 高考信息列表  **/
  const getSchoolDetailRelList = async (id: string) => {
    if (!id) return;
    let schoolId = id.toString();
    const res = await request<{
      code: number;
      data: {
        list: DataSourceType[];
      };
    }>('/web/schoolDetailRel/list', {
      method: 'GET',
      params: {
        pageNum: 1,
        pageSize: 1000,
  ...
  const getDetailsData = async (id: string) => {
    const res = await request('/web/schoolDetails/' + id, {
      method: 'GET',
    });
    if (res && res.code === 200) {
      form.setFieldsValue({
        ...res.data,
        viewSchoolId: res.data.viewSchoolId ? res.data.viewSchoolId.split(',') : undefined,
      });
      setDataList(res.data);
      if (res.data.provinceCode) {
        const list = await getRoleList(res.data.provinceCode);
        setCityList(list);
      }
      if (res.data.cityCode) {
        const list = await getRoleList(res.data.cityCode);
        setDistrictList(list);
      }
      if (res.data.id) {
        getSchoolDetailRelList(res.data.id.split(','));
      }
    }
  };
