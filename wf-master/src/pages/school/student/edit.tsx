...
const AdvancedForm: FC<Record<string, any>> = (props) => {
  ...
  const [detailsList, setDetailsList] = useState<GithubIssueItem>({});
  ...
  const onFinish = async (values: Record<string, any>) => {
    ...
    if (values.pickerLists) {
      picker = values.pickerLists[0].url.replace('/question-api', '');
    }
    ...
    const res = await request<API.GeneralInterface>('/web/schoolStudent', {
      ...
      data: {
        ...
        ...values,
        picker,
        pickerLists: undefined,
        version: detailsList.version,
      },
    });
    if (res && res.code === 200) {
  ...
  const getDictDataPage = async (it: { dictType: string; data: (arg0: any) => void }) => {
    ...
    if (res && res.code === 200 && res.data) {
      const list = res.data.list.map((item: { dictLabel: any; id: any }) => {
        return {
  ...
  const getDetailsList = async () => {
    ...
    }>('/web/schoolStudent/' + props.match.params.id, {
      method: 'GET',
    });
    if (res && res.code === 200) {
      let pickerLists: { status: string; url: string }[] = [];
      if (res.data.picker) {
        pickerLists = [
          {
            status: 'done',
