...
export default () => {
  ...
  const deleteCl = async (id: string) => {
    const res = await request<API.GeneralInterface>('/web/schoolMock/' + id, {
      method: 'DELETE',
    });
    if (res && res.code === 200) {
  ...
  const columns: ProColumns<GithubIssueItem>[] = [
    ...
    },
    {
      ...
      key: 'option',
      render: (text, record) => [
        <Link to={'/educational/simulation/set/' + record.id}>设置题目</Link>,
        <a
          key="editable"
          onClick={() => {
            setTypes('edit');
  ...
  const getDataList = async (datas: { pageNum: number; pageSize: number }) => {
    const res = await request<TableListData>('/web/schoolMock/list', {
      method: 'GET',
      params: {
        ...datas,
      },
  ...
  const onFinish = async (value: GithubIssueItem) => {
    ...
    if (repeatIs) return;
    setRepeatIs(true);
    const urls = types === 'add' ? '/web/schoolMock/add' : '/web/schoolMock';
